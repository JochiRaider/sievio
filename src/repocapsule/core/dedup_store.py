    ...
    def _check_and_add_impl(
        ...
        *,
        ...
        if band_keys:
            ...
            cur = conn.execute(f"SELECT DISTINCT doc_id FROM lsh_index WHERE band_key IN ({placeholders})", band_keys)
        ...
        if candidates:
            ...
            for cand_id, blob in cur.fetchall():
                ...
                eq = sum(1 for a, b in zip(sig, cand_sig) if a == b)
                if eq > best_eq:
                    best_eq = eq
                    best_id = cand_id
        ...
        if add_if_missing:
            try:
                ...
                log.debug("Doc %s already present in dedup store %s", doc_id, self.db_path, exc_info=True)
        if commit and add_if_missing:
            conn.commit()
