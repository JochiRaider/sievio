...
JSONL record builders and lightweight metadata utilities.

Responsibilities
----------------
- Decide **kind** ("code" vs "doc") and **lang** from file extension.
- Compute stable content **hash** (sha256 of UTF‑8 text) and size in **bytes**.
- Provide an **estimated token** count (via `repocapsule.chunk.approx_token_count`).
- Optionally derive a coarse **category** (e.g., mde/sentinel) from relative path.
- Assemble a canonical JSONL record: `{ "text": ..., "meta": {...} }`.

Stdlib‑only. No I/O here.
"""
