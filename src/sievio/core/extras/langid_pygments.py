...
class PygmentsCodeLanguageDetector:
    ...
    def __init__(self, cfg: LanguageConfig | None = None) -> None:
        try:
            from pygments.lexers import guess_lexer, guess_lexer_for_filename  # type: ignore
        except Exception as exc:  # pragma: no cover - optional dependency
            raise ImportError("Pygments backend requires the 'pygments' package.") from exc
    ...
    def detect_code(self, text: str, *, filename: str | None = None) -> CodeLanguagePrediction | None:
        ...
        score = 0.7
        return CodeLanguagePrediction(lang=lang, score=score, reliable=False, score_kind="probability", backend="pygments")
